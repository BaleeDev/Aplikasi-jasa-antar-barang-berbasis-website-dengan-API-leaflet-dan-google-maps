...
if(isset($_POST['bsimpan'])){
   ...
   $pass = md5($_POST['pass']);
   if($nomor == "" || $pass ==""){
    ?> <script type="text/javascript">alert("Nomor / Password Tidak Boleh Kosong");</script> <?php
   }else{
     $sql = "SELECT * FROM tbl_kurir WHERE nohp='$nomor' AND password='$pass'";
    $result = mysqli_query($conn, $sql);
    if ($result->num_rows > 0) {
        $row = mysqli_fetch_assoc($result);
        $_SESSION['kurir'] = $row['id_kurir'];
        echo "<script>
            alert
            document.location='cek.php';
            </script>";
    } else {
        echo "<script>alert('Nomor atau password Anda salah. Silahkan coba lagi!')</script>";
    }
   }
...
<div class="card card-success mt-3">
              <div class="card-header">
                <h3 class="card-title">Form Masuk Mitra AtongBae</h3>
              </div>
              <!-- /.card-header -->
              <!-- form start -->
              <form action="" method="post">
                <div class="card-body">
                  ...
                  <input type="text" name="nomor" required class="form-control form-control-border" placeholder="83129108633">
                </div>
                ...
                <div class="card-footer text-center">
                  <button type="submit" name="bsimpan" class="btn btn-success">Masuk</button>
                </div>
              </form>
            </div>
            <!-- /.card -->
