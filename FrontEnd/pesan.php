...
function cekstatus($idUser,$conn){
      $query = mysqli_query($conn, "SELECT * FROM tbl_detailtransaksi WHERE id_transaksi = '$idUser' ");    
...
function cek($idUser,$status,$conn){
    ...
    // function untuk mengambil id pesanan
    function ambiIdPesanan($idTransaksi,$conn){
        $query = mysqli_query($conn, "SELECT * FROM tbl_pesan WHERE id_transaksi = '$idTransaksi'");  
      $data = mysqli_fetch_array($query);
...
if(isset($_SESSION['user'])){
    ...
    $cekstatus = cekstatus($vcek,$conn);
    if($cekstatus != "Sedang di Proses"){
    $id_pesan = ambiIdPesanan($vcek,$conn);
    
    // mengambil idtransaksi berdasarkan id user dan status 'Pesan'
    ...
    <section id="features" class="features">
      <div class="container">
        ...
        <div class="section-title" data-aos="fade-up">
          <p>Form Lengkapi Detail Pengiriman Anda</p>
        </div>
          <div class="container">
        ...
        <div class="row">
            ...
            <div class="col-md-12">
                <form action="proses_simpan.php" method="post">
                    <small id="emailHelp" class="form-text text-muted"><i>Isi Detail Data Penerima Yang Di Butuhkan </i>.<strong>(sesuai dengan nomor pesanan!!)</strong></small>
                ...
                ?>
                  ...
                  <div class="form-group col-md-4 col-6 mb-3">
                      <small for="">Nama Penerima <?=$i?> </small>
                    <input type="text" class="form-control"  id="nama<?=$i?>" name="nama<?=$i?>"  placeholder="Nama">
                    </div>
                    <div class="form-group col-md-4 col-6 mb-3">
                        <small for="">No Hp <?=$i?> </small>
                    <input type="text" class="form-control"  id="nohp<?=$i?>" name="nohp<?=$i?>"  placeholder="Masukkan no hp yang bisa di hubungi ke-<?=$i?>">
                    </div>
                    <div class="form-group col-md-4 col-12 mb-3">
                        <small for="">Alamat <?=$i?> </small>
                    <textarea class="form-control" id="alamat<?=$i?>"  name="alamat<?=$i?>" rows="3"></textarea>
                  </div>
               <?php }?>
               <input type="hidden" name="idPesan" value="<?=$id_pesan?>">
               <input type="hidden" name="batas" value="<?=$no?>">
               <input type="hidden" name="idTransaksi" value="<?=$vcek?>">
               </div>
               <div class="text-center">
               <button type="submit" class="btn btn-outline-success btn-sm">Pesan</button>
               </div>
                </form>
            </div>
        </div>
...
<?php }else{
    echo "<script>location='cek_ongkir.php';</script>";
}
}else{
